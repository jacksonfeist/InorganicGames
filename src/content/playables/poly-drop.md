...
mechanic: 'Tap to release the next polygon — stack it into the bin before the pile topples out.'
...
from the pile, not from the controls — the player is never fighting the interface,
...
that runs offline in any mobile browser. That is the same constraint an ad unit runs
